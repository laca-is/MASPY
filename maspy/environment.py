...
class Environment(metaclass=EnvironmentMultiton):
    ...
    def _extend_fact(self, name: str, data: str, role='any'):
        ...
        try:
            ...
            print(f"{self._my_name}> Fact {name}:{type(self._facts[role][name])} can't extend {type(data)}")
    ...
    def _reduce_fact(self, name, data, role='any'):
        ...
        try:
            ...
            print(f"{self._my_name}> Fact {name}:{type(self._facts[role][name])} doesn't contain {data}")
    ...
    def _rm_fact(self, del_name: str, del_data: Any = None, del_role: Optional[str] = None):
        if del_role is None:
